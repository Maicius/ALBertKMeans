...
from src.albert import modeling, optimization
from src.albert import tokenization
...
def read_csv(input_file):
    ...
    lines = [[jlabel[i], fact[i]] for i in range(len(jlabel)) if type(fact[i]) == str]
    return lines
...
class DataProcessor(object):
    ...
    def _read_csv(cls, input_file, content_col=None, label_col=None):
        ...
        lines = [[labels[i], job_fact[i]] for i in range(len(labels)) if type(job_fact[i]) == str]
    ...
    @classmethod
    def change_label_to_id(self, x, label):
        """
        将字符形式的标签转换为one hot形式的标签
        :param x:
        :return:
        """
        return [0]
...
class ClassifyProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir, content_col=None, label_col=None):
        """See base class."""
        return self._create_examples(
            self._read_csv(data_dir, content_col, label_col), "train")

    def get_train_example_length(self, data_dir):
        return len(self._read_csv(os.path.join(data_dir, hp.train_data)))

    def get_train_labels(self, data_dir, label_col):
        df = load_csv(data_dir, header=0)
        label_list = df[label_col].apply(lambda x: self.change_label_to_id(x, label_col))
    ...
    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_csv(os.path.join(data_dir, hp.val_data)), "val")

    def get_test_examples(self, data_dir, content_col=None, label_col=None):
        """See base class."""
        return self._create_examples(
            self._read_csv(os.path.join(data_dir, hp.test_data), content_col, label_col), "test")

    def get_test_raw_example(self):
        return self.read_raw_csv(os.path.join(data_dir, hp.test_data))
...
def get_features(data_dir, content_col):
    # Load train data
    train_examples = processor.get_train_examples(data_dir, content_col)
    # Get onehot feature
    seq_len = hp.seq_len
    features = convert_examples_to_features(train_examples, label_list, seq_len, tokenizer,
                                            task_name='classify')
...
def get_features_from_example(examples, seq_len):
...
def preprocess_lxqj_content(x):
    "['21：认罪态度好', '22：犯罪后采取补救措施降低损失']"
    x = x[1:-1]
    return x


def get_train_target_label(data_dir, label_col):
    return processor.get_train_labels(data_dir, label_col)


def get_features_test(content_col, label_name=None):
    # Load test data
    train_examples = processor.get_test_examples(data_dir, content_col=content_col, label_col= label_name)
...
def get_feature_test(sentence):
    ...
    feature = convert_single_example(0, example, label_list, hp.fact_sequence_length, tokenizer, task_name='classify')
